# 对话引擎核心：管理多轮回访流程（yes/no + 地址核对），支持超时重试和 LLM 意图分类。
...
# 两种常用流程组合
DEFAULT_FLOW_STEPS = tuple(DEFAULT_STEPS[:2])   # 仅 yes/no 阶段
ADDRESS_ONLY_STEPS = (DEFAULT_STEPS[2],)         # 仅地址核对阶段


class Speaker(Protocol):
    """语音播报协议：实现类需提供 speak 方法。"""
    def speak(self, text: str) -> None: ...


class Listener(Protocol):
    """语音监听协议：listen_once 返回 {"timed_out": bool, "text": str}。"""
...
class DialogueEngine:
    ...
    def _handle_yes_no_step(
        ...
        """yes/no 阶段处理：意图分类 -> 成功则推进，否则按重试次数决定是追问还是标 unclear。"""
    ...
    def _handle_address_step(
        ...
        """地址阶段处理：调用高德 API 搜索 -> 匹配则确认，不匹配则追问。"""
